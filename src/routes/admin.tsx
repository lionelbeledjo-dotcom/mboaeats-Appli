...
function AdminLayout() {
  ...
  return (
    <SidebarProvider>
      <div data-admin-shell className="flex h-dvh w-full bg-background text-foreground">
        <AdminSidebar />
        <div className="flex-1 flex flex-col min-w-0">
          <AdminHeader adminInfo={adminInfo} loading={loading} isAdmin={isAdmin} />
          <main data-admin-scroll-container className="flex-1 min-h-0 overflow-x-hidden overflow-y-auto">
            <Outlet />
          </main>
        </div>
      </div>
